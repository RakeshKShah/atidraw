...
COOKIE_JAR="/tmp/consecutive_upload_allowed_different_user_cookie_${CASE_SUFFIX}.txt"
GIVEN_HEADERS="/tmp/consecutive_upload_allowed_different_user_given_headers_${CASE_SUFFIX}.txt"
GIVEN_BODY="/tmp/consecutive_upload_allowed_different_user_given_body_${CASE_SUFFIX}.txt"
WHEN_HEADERS="/tmp/consecutive_upload_allowed_different_user_when_headers_${CASE_SUFFIX}.txt"
WHEN_BODY="/tmp/consecutive_upload_allowed_different_user_when_body_${CASE_SUFFIX}.txt"
DRAWING_FILE="/tmp/consecutive_upload_allowed_different_user_drawing_${CASE_SUFFIX}.jpg"
...
printf '\377\330\377\341Different user JPEG %s\377\331' "$CASE_SUFFIX" > "$DRAWING_FILE"

# Given
echo "STEP: Given — create a fresh authenticated session and prepare a valid JPEG drawing"
...
# When
echo "STEP: When — upload a valid JPEG drawing expected to succeed for a different effective user"
...
# Then
echo "STEP: Then — upload succeeds and returns generated image content"
...
[ -s "$WHEN_BODY" ] || { echo "ASSERTION_FAILED: expected non-empty response body"; exit 1; }

# Cleanup
echo "STEP: Cleanup — remove temporary files for this test invocation"

printf 'CODEVALID_TEST_ASSERTION_OK:consecutive_upload_allowed_different_user\n'
