...
COOKIE_JAR="/tmp/happy_path_alt_text_generated_cookie_${CASE_SUFFIX}.txt"
GIVEN_HEADERS="/tmp/happy_path_alt_text_generated_given_headers_${CASE_SUFFIX}.txt"
GIVEN_BODY="/tmp/happy_path_alt_text_generated_given_body_${CASE_SUFFIX}.txt"
WHEN_HEADERS="/tmp/happy_path_alt_text_generated_when_headers_${CASE_SUFFIX}.txt"
WHEN_BODY="/tmp/happy_path_alt_text_generated_when_body_${CASE_SUFFIX}.txt"
DRAWING_FILE="/tmp/happy_path_alt_text_generated_drawing_${CASE_SUFFIX}.jpg"
...
printf '\377\330\377\341CodeValid JPEG %s\377\331' "$CASE_SUFFIX" > "$DRAWING_FILE"

# Given
echo "STEP: Given — bootstrap an authenticated user session and prepare a valid JPEG drawing"
...
# When
echo "STEP: When — upload a valid JPEG drawing through POST /api/upload"
...
# Then
echo "STEP: Then — upload completes successfully and returns generated image bytes"
...
[ -s "$WHEN_BODY" ] || { echo "ASSERTION_FAILED: expected non-empty response body for generated image"; exit 1; }

# Cleanup
echo "STEP: Cleanup — remove temporary files for this test invocation"

printf 'CODEVALID_TEST_ASSERTION_OK:happy_path_alt_text_generated\n'
