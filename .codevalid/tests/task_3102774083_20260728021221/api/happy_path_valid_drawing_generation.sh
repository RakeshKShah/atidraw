...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="happy_path_valid_drawing_generation"
...
echo "STEP: Given — bootstrap authenticated anonymous session and prepare valid drawing upload"
...
echo "STEP: When — upload valid drawing to POST /api/generate"
...
code="$(curl -sS -D "$GEN_HEADERS" -o "$GEN_BODY" -w '%{http_code}' -c "$COOKIE_JAR" -b "$COOKIE_JAR" -F "drawing=@${DRAWING_FILE};type=image/png;filename=drawing.png" "$BASE_URL/api/generate")"
echo "RESPONSE_HEADERS:"
cat "$GEN_HEADERS"
echo "RESPONSE_BODY:"
cat "$GEN_BODY"
...
echo "STEP: Then — verify successful generated image response"
...
[ -s "$GEN_BODY" ] || { echo "ASSERTION_FAILED: expected non-empty response body"; exit 1; }

# Cleanup — undo Given side effects
echo "STEP: Cleanup — remove temporary files only"

echo "CODEVALID_TEST_ASSERTION_OK:happy_path_valid_drawing_generation"
