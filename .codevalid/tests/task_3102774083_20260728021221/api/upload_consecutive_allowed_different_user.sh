...
printf '\377\330\377\340JFIF\000allowed-different-%s' "$CASE_SUFFIX" > "$DRAWING_FILE"
printf 'multipart upload expecting success after different user using @%s\n' "$DRAWING_FILE" > "$REQ_BODY_LOG"

# Given
echo "STEP: Given — prepare authenticated upload after a different user uploaded last"
echo "PREREQ: caller must provide authenticated session and environment state where last drawing belongs to another user"
