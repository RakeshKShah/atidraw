...
printf '\377\330\377\340JFIF\000ai-fallback-%s' "$CASE_SUFFIX" > "$DRAWING_FILE"
printf 'multipart upload expecting AI description graceful fallback @%s\n' "$DRAWING_FILE" > "$REQ_BODY_LOG"

# Given
echo "STEP: Given — prepare authenticated jpeg expecting description-model failure fallback"
echo "PREREQ: caller must provide authenticated session; environment should make description generation fail while endpoint catch fallback remains active"
...
# When
echo "STEP: When — POST drawing to /api/upload expecting fallback to empty description"
...
# Then
echo "STEP: Then — assert upload still succeeds despite description failure"
