...
printf 'multipart upload drawing=@%s expecting moderation rejection when AI description contains prohibited text\n' "$DRAWING_FILE" > "$REQ_BODY_LOG"

# Given
echo "STEP: Given — bootstrap authenticated session and jpeg payload for moderation scenario"
...
# When
echo "STEP: When — perform POST /api/upload expecting moderation rejection"
echo "REQUEST_HEADERS:"
...
# Then
echo "STEP: Then — assert inappropriate content is rejected when moderation triggers"
...
# Cleanup
echo "STEP: Cleanup — no public API cleanup available for rejected upload scenario"
