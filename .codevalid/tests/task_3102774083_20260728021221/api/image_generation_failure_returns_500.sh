...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="image_generation_failure_returns_500"
...
echo "STEP: Given — bootstrap authenticated anonymous session and prepare drawing while image generation may fail"
...
echo "PREREQ: hub-ai is provisioner none in dependencies.json; forced empty-files Gemini response cannot be deterministically mocked in patch-only testgen"

# When — perform the action under test
echo "STEP: When — upload drawing to POST /api/generate and observe image generation failure behavior"
...
code="$(curl -sS -D "$GEN_HEADERS" -o "$GEN_BODY" -w '%{http_code}' -c "$COOKIE_JAR" -b "$COOKIE_JAR" -F "drawing=@${DRAWING_FILE};type=image/png;filename=sketch.png" "$BASE_URL/api/generate")"
echo "RESPONSE_HEADERS:"
cat "$GEN_HEADERS"
echo "RESPONSE_BODY:"
cat "$GEN_BODY"
...
echo "STEP: Then — verify 500 error with Failed to generate image message"
[ "$code" = "500" ] || { echo "ASSERTION_FAILED: expected HTTP 500 when generated image is missing got ${code}"; exit 1; }
grep -F 'Failed to generate image' "$GEN_BODY" >/dev/null 2>&1 || { echo "ASSERTION_FAILED: expected error message 'Failed to generate image'"; exit 1; }

# Cleanup — undo Given side effects
echo "STEP: Cleanup — remove temporary files only"

echo "CODEVALID_TEST_ASSERTION_OK:image_generation_failure_returns_500"
