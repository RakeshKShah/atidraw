...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="upload_blob_list_empty"
...
printf '\377\330\377\340JFIF\000first-upload-%s' "$CASE_SUFFIX" > "$DRAWING_FILE"
printf 'multipart upload expecting success with empty drawings list @%s\n' "$DRAWING_FILE" > "$REQ_BODY_LOG"

# Given
echo "STEP: Given — prepare authenticated first-upload scenario"
echo "PREREQ: caller must provide authenticated session and environment where blob.list for drawings prefix returns zero blobs"
...
# When
echo "STEP: When — POST first drawing to /api/upload"
...
# Then
echo "STEP: Then — assert first upload succeeds when no drawings exist"
[ "$code" = "200" ] || { echo "ASSERTION_FAILED: expected HTTP 200 got ${code}"; exit 1; }
grep -E 'pathname|drawings/|description|aiImage|url' "$RESP_BODY" >/dev/null 2>&1 || { echo "ASSERTION_FAILED: expected response body to include successful first-upload metadata"; exit 1; }

# Cleanup
echo "STEP: Cleanup — no cleanup available through public API"
echo "CODEVALID_TEST_ASSERTION_OK:upload_blob_list_empty"
