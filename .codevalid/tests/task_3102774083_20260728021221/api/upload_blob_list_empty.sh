...
printf 'multipart upload first-drawing scenario drawing=@%s\n' "$DRAWING_FILE" > "$REQ_BODY_LOG"

# Given
echo "STEP: Given — bootstrap authenticated session and valid jpeg payload for first-upload scenario"
...
# Then
echo "STEP: Then — assert upload succeeds when blob list is empty or otherwise not blocking this new session"
[ "$code" = "200" ] || { echo "ASSERTION_FAILED: expected HTTP 200 got ${code}"; exit 1; }
grep -E 'pathname|drawings/|description|aiImage|url' "$RESP_BODY" >/dev/null 2>&1 || { echo "ASSERTION_FAILED: expected response body to include successful upload metadata"; exit 1; }
