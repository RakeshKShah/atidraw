...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="upload_content_moderation_clean"
...
printf '\377\330\377\340JFIF\000moderation-clean-%s' "$CASE_SUFFIX" > "$DRAWING_FILE"
printf 'multipart upload expecting clean moderation pass @%s\n' "$DRAWING_FILE" > "$REQ_BODY_LOG"

# Given
echo "STEP: Given — prepare authenticated clean jpeg upload"
echo "PREREQ: caller must provide authenticated session and environment where AI description does not contain prohibited text"
...
# When
echo "STEP: When — POST clean drawing to /api/upload"
...
# Then
echo "STEP: Then — assert clean content upload succeeds"
[ "$code" = "200" ] || { echo "ASSERTION_FAILED: expected HTTP 200 got ${code}"; exit 1; }
grep -E 'pathname|drawings/|description|aiImage|url' "$RESP_BODY" >/dev/null 2>&1 || { echo "ASSERTION_FAILED: expected response body to include successful drawing metadata"; exit 1; }

# Cleanup
echo "STEP: Cleanup — no cleanup available through public API"
echo "CODEVALID_TEST_ASSERTION_OK:upload_content_moderation_clean"
