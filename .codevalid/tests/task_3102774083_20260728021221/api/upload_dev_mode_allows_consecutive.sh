...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="upload_dev_mode_allows_consecutive"
...
printf '\377\330\377\340JFIF\000dev-mode-%s' "$CASE_SUFFIX" > "$DRAWING_FILE"
printf 'multipart upload expecting dev-mode bypass @%s\n' "$DRAWING_FILE" > "$REQ_BODY_LOG"

# Given
echo "STEP: Given — prepare authenticated upload in development mode"
echo "PREREQ: caller must provide authenticated session and app must run with import.meta.dev true while same user uploaded last drawing"
...
# When
echo "STEP: When — POST drawing to /api/upload in development mode"
...
# Then
echo "STEP: Then — assert development mode bypasses consecutive-upload block"
[ "$code" = "200" ] || { echo "ASSERTION_FAILED: expected HTTP 200 got ${code}"; exit 1; }
grep -E 'pathname|drawings/|description|aiImage|url' "$RESP_BODY" >/dev/null 2>&1 || { echo "ASSERTION_FAILED: expected successful upload metadata response"; exit 1; }

# Cleanup
echo "STEP: Cleanup — no cleanup available through public API"
echo "CODEVALID_TEST_ASSERTION_OK:upload_dev_mode_allows_consecutive"
