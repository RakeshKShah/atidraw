...
printf 'multipart upload drawing=@%s expecting development-mode consecutive bypass\n' "$DRAWING_FILE" > "$REQ_BODY_LOG"

# Given
echo "STEP: Given — bootstrap authenticated session and perform an initial upload"
...
echo "PREREQ: performing first upload using same session"
echo "REQUEST_HEADERS:"
...
# When
echo "STEP: When — perform second POST /api/upload with same session"
echo "REQUEST_HEADERS:"
printf '%s\n' 'Content-Type: multipart/form-data (curl -F)'
printf 'Cookie jar: %s\n' "$COOKIE_JAR"
echo "REQUEST_BODY:"
...
# Then
echo "STEP: Then — assert development mode would allow consecutive uploads"
