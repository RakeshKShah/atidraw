...
printf '\377\330\377\340JFIF\000blocked-%s' "$CASE_SUFFIX" > "$DRAWING_FILE"
printf 'multipart upload expecting consecutive-upload block using @%s\n' "$DRAWING_FILE" > "$REQ_BODY_LOG"

# Given
echo "STEP: Given — prepare authenticated upload expecting same-user consecutive block"
echo "PREREQ: caller must provide authenticated session already representing the same last uploader in non-dev mode"
[ -s "$DRAWING_FILE" ] || { echo "ASSERTION_FAILED: expected drawing file to exist"; exit 1; }
if [ ! -s "$COOKIE_JAR" ] && [ -z "$AUTH_HEADER" ]; then
  echo "ASSERTION_FAILED: this scenario requires COOKIE_JAR or AUTH_HEADER for an authenticated same-user session"
  exit 1
fi

# When
echo "STEP: When — POST drawing to /api/upload as same last uploader"
...
# Then
echo "STEP: Then — assert same-user consecutive upload is rejected"
