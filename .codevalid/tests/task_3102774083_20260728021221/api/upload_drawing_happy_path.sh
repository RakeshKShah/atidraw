...
printf 'multipart field drawing=@%s;type=image/jpeg;filename=%s.jpg\n' "$DRAWING_FILE" "$ITEM_ID" > "$REQ_BODY_LOG"

# Given
echo "STEP: Given — bootstrap authenticated session and valid jpeg payload"
echo "PREREQ: creating synthetic jpeg payload for self-contained upload test"
...
# When
echo "STEP: When — perform authenticated POST /api/upload"
echo "REQUEST_HEADERS:"
...
# Then
echo "STEP: Then — assert successful upload metadata is returned"
[ "$code" = "200" ] || { echo "ASSERTION_FAILED: expected HTTP 200 got ${code}"; exit 1; }
grep -E 'pathname|drawings/|description|aiImage|url' "$RESP_BODY" >/dev/null 2>&1 || { echo "ASSERTION_FAILED: expected response body to include drawing metadata"; exit 1; }
