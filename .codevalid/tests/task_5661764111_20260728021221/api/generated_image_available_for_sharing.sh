...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="generated_image_available_for_sharing"
...
printf 'fake-png-sharing-%s' "$CASE_SUFFIX" > "$DRAWING_FILE"
...
echo "STEP: When — request generated image bytes for downstream client use"
echo "REQUEST_HEADERS: Cookie jar=$COOKIE_JAR; Content-Type: multipart/form-data"
echo "REQUEST_BODY: drawing=@$DRAWING_FILE;type=image/png;filename=portrait.png"
response_code=$(curl -sS -D "$RESPONSE_HEADERS" -o "$RESPONSE_BODY" -w '%{http_code}' -b "$COOKIE_JAR" -c "$COOKIE_JAR" -X POST -F "drawing=@${DRAWING_FILE};type=image/png;filename=portrait.png" "$BASE_URL/api/generate")
echo "RESPONSE_HEADERS:"
...
echo "STEP: Then — binary response is suitable for display, storage, and sharing flows"
...
[ -s "$RESPONSE_BODY" ] || { echo "ASSERTION_FAILED: expected non-empty binary response body"; exit 1; }
body_size=$(wc -c < "$RESPONSE_BODY" | tr -d '[:space:]')
[ "$body_size" -gt 0 ] || { echo "ASSERTION_FAILED: expected positive byte length got ${body_size}"; exit 1; }

echo "STEP: Cleanup — no persistent cleanup required"
echo "CODEVALID_TEST_ASSERTION_OK:generated_image_available_for_sharing"
