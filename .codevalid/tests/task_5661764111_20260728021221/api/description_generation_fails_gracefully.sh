...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="description_generation_fails_gracefully"
...
printf 'fake-png-description-fallback-%s' "$CASE_SUFFIX" > "$DRAWING_FILE"
...
echo "STEP: When — submit drawing while description generation may fail gracefully"
echo "REQUEST_HEADERS: Cookie jar=$COOKIE_JAR; Content-Type: multipart/form-data"
echo "REQUEST_BODY: drawing=@$DRAWING_FILE;type=image/png;filename=artwork.png"
response_code=$(curl -sS -D "$RESPONSE_HEADERS" -o "$RESPONSE_BODY" -w '%{http_code}' -b "$COOKIE_JAR" -c "$COOKIE_JAR" -X POST -F "drawing=@${DRAWING_FILE};type=image/png;filename=artwork.png" "$BASE_URL/api/generate")
echo "RESPONSE_HEADERS:"
...
echo "STEP: Then — image generation still succeeds and description header exists"
[ "$response_code" = "200" ] || { echo "ASSERTION_FAILED: expected HTTP 200 got ${response_code}"; exit 1; }
grep -iq '^x-description:' "$RESPONSE_HEADERS" || { echo "ASSERTION_FAILED: expected x-description header even on graceful description fallback"; exit 1; }
...
echo "CODEVALID_TEST_ASSERTION_OK:description_generation_fails_gracefully"
